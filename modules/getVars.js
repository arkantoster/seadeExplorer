  ...
  try {
    ...
    while (isThereMore) {
      ...
      const {
        ...
        `${SEADE_URL}/variavel?offset=${offset}&limit=${limit}`
      );

      for (const [key, value] of Object.entries(variavel)) {
        const { codigo, nome, unidade, periodo, definicao, nota, fonte } =
          value;
        if (periodo.checkYears(2018) && periodo.checkYears(2019)) {
          added++;
          await SQLite.run(
            `INSERT INTO variaveis VALUES(${codigo}, '${nome}', '${unidade}', '${periodo}', '${definicao}', '${[
              ...nota,
            ].join("")}', '${[...fonte].join("")}')`
          );
        }
